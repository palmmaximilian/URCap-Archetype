...
if [[ -z $1 ]] || [[ "$1" != "-t" ]]; then
	...
	# Ask if the user wants an installation node
	exec 3>&1
	INSTALLATION_NODE_CHOICE=$(dialog --yesno "Include an Installation Node, Program Node and Toolbar in the project?" 10 60 2>&1 1>&3)
	exec 3>&-
	full_install=$?  # 0=Yes, 1=No
...
if [ ! -f "archetype-1.16.0.jar" ]; then
  echo "Error: archetype-1.16.0.jar not found in current directory!"
  exit 1
fi

if ! jar tf archetype-1.16.0.jar &>/dev/null; then
  echo "Error: archetype-1.16.0.jar is corrupted!"
  exit 1
fi

mvn install:install-file \
  -Dfile="$(pwd)/archetype-1.16.0.jar" \
...
ls -la ~/.m2/repository/com/ur/urcap/archetype/1.16.0/
